...
const char* DebugRenderer::FONT_FILE = "arial.ttf";
...
void DebugRenderer::DrawPoint(const b2Vec2& p, float32 size, const b2Color& color)
{
	...
	sf::Color c(color.r*255., color.g*255., color.b*255.);
	ThickPointShape point(window, sf::Vector2f(p.x, -p.y));
...
void DebugRenderer::vDrawString(int x, int y, const char* string, va_list arg, const b2Color& color)
{
	...
	float xRatio = 0.5 * window->getView().getSize().x / (float)window->getSize().x;
	float yRatio = 0.5 * window->getView().getSize().y / (float)window->getSize().y;
	sf::Text text;

	text.setFont(_font);
	text.setString(buffer);
	text.setPosition(window->mapPixelToCoords(sf::Vector2i(realX, realY)));
