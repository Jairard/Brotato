...
namespace DnaSystem
{
	const char* Collector::c_tag = "DNACollector";
	const size_t Collector::c_framesToSkipForDNAInfo      = Debug::AbstractCallstack::c_defaultSkippedFrameCount + 6;
	const size_t Collector::c_framesToSkipForOrganismInfo = Debug::AbstractCallstack::c_defaultSkippedFrameCount + 7;

	// DNACollector::DNAInfo
	Collector::DNAInfo::DNAInfo(BaseDNA& _dna):
		  dna(_dna)
		, organism(nullptr)
#ifdef POT_DEBUG
	...
	Collector::DNAInfo& Collector::DNAInfo::operator=(const DNAInfo& other)
	{
		dna = other.dna;
	...
	Collector::OrganismInfo::OrganismInfo():
		  ptr(nullptr)
#ifdef POT_DEBUG
		, alive(false)
		, type(typeid(void))
	...
	Collector::OrganismInfo::OrganismInfo(const BaseOrganism* _ptr, const std::type_info& _type, bool _alive):
		  ptr(_ptr)
#ifdef POT_DEBUG
		, alive(_alive)
		, type(_type)
	...
	void Collector::dump(const char* tag)
	{
	...
	void Collector::dump_internal(std::ostream& outStream, bool displayCallstacks) const
	{
#ifndef POT_DEBUG
		...
		for (OrganismContainer::const_iterator ptrIt = m_organismContainer.begin(); ptrIt != m_organismContainer.end(); ++ptrIt)
		{
			...
			const CollectorTimestamp& aliveTimestamp = infoForPtr.first;
			...
			{
				const CollectorTimestamp& t = organismIt->first;
	...
	const char* Collector::tag()
	{
		return c_tag;
	}

	void Collector::setFullCheckMode(bool enabled)
	{
#ifdef POT_DEBUG
		Collector* const instance = Collector::instanceIFP();
		if (instance != nullptr)
			instance->m_checker.setFullCheckMode(enabled);
#endif
	}

	size_t Collector::memorySize() const
	{
		size_t size = 0;
		const size_t timestampSize = sizeof(CollectorTimestamp);
	...
	void Collector::registerDNA(BaseDNA& dna, const std::type_info& organismType)
	{
		...
		CollectorTimestamp t = CollectorTimestamp::c_invalid;
		...
		{
			const CollectorTimestamp& aliveTimestamp = organismIt->second.first;
	...
	void Collector::unregisterDNA(BaseDNA& dna)
	{
		const BaseOrganism* organism = dna.organism();
		...
		CollectorTimestamp t = fetchTimestamp(dna, dnaTypeIndex, &organism);
	...
	void Collector::notifyOrganismDeath(const BaseOrganism& organism)
	{
		...
		CollectorTimestamp& aliveTimestampRef = infoForPtr.first;
#ifdef POT_DEBUG
...
#endif
		CollectorTimestamp t = aliveTimestampRef;
		ASSERT_DEBUG(t.isValid());
	...
	/* O(OrgC) */
	const Collector::OrganismInfo* Collector::getOrganismInfoFromTimestamp(const CollectorTimestamp& t) const
	{
	...
	const CollectorTimestamp& Collector::createOrganismEntry(const BaseOrganism* ptr, const std::type_info& type)
	{
		const CollectorTimestamp& t = m_timestamp++;
		...
		CollectorTimestamp& aliveTimestamp = infoForPtr.first;
		aliveTimestamp = t;
	...
	/* O(1) / O(OrgWC) | O(OrgWC) + O(TC) */
	void Collector::clearOrganismInfoForTimestamp(const BaseOrganism* ptr, const CollectorTimestamp& t)
	{
	...
	CollectorTimestamp Collector::fetchTimestamp(const BaseDNA& dna, const std::type_index& dnaTypeIndex, const BaseOrganism** outOrganism) const
	{
		const BaseOrganism* organism = dna.organism();
		...
		if (organism == nullptr) // Organism is dead, we have to fetch the organism ptr in the DNA container
		{
			...
			ASSERT_NOT_REACHED();
			return CollectorTimestamp::c_invalid;
		}
		else // Organism is alive, we can directly look in the organism container
		{
			...
			const CollectorTimestamp& t = infoForPtr.first;
			ASSERT_DEBUG(t.isValid());
	...
	/* O(OrgWC) */
	size_t Collector::organismCount() const
	{
		size_t count = 0;
		for (OrganismContainer::const_iterator it = m_organismContainer.begin(); it != m_organismContainer.end(); ++it)
	...
	/* O(DnaC) */
	size_t Collector::dnaCount() const
	{
		size_t count = 0;
...
#ifdef POT_DEBUG
	/* O(1) / O(OrgWC) + O(TC) */
	void Collector::checkOrganismStatus(const BaseOrganism* ptr, const CollectorTimestamp& t, bool shouldBeAlive) const
	{
	...
	void Collector::checkIntegrityIFN()
	{
